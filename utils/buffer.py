...
class Buffer:
    ...
    def __init__(self, buffer_size, device, n_tasks=None, mode='reservoir'):
        ...
        self.attributes = ['examples', 'labels', 'logits', 'task_labels', 'logits_meta']
    ...
    def init_tensors(self, examples: torch.Tensor, labels: torch.Tensor,
                     logits: torch.Tensor, task_labels: torch.Tensor, logits_meta: torch.Tensor) -> None:
        """
    ...
    def add_data(self, examples, labels=None, logits=None, task_labels=None, logits_meta=None):
        """
        ...
        if not hasattr(self, 'examples'):
            self.init_tensors(examples, labels, logits, task_labels, logits_meta)
        ...
        for i in range(examples.shape[0]):
            ...
            if index >= 0:
                ...
                if logits_meta is not None:
                    self.logits_meta[index] = logits_meta[i].to(self.device)

    def add_data_task(self, examples, labels=None, logits=None, task_labels=None, logits_meta=None, task=0):
        """
        Adds the data to the memory buffer according to the reservoir strategy.
        :param examples: tensor containing the images
        :param labels: tensor containing the labels
        ...
        for i in range(examples.shape[0]):
            index = reservoir(self.num_seen_examples, self.buffer_size)
            self.num_seen_examples += 1 * (task + 1)
            if index >= 0:
                self.examples[index] = examples[i].to(self.device)
                if labels is not None:
                    self.labels[index] = labels[i].to(self.device)
    ...
    def get_balanced_data(self, size_per_cls: int, transform: transforms = None) -> Tuple:
        """
        Random samples a batch of size items.
        :param size_per_cls: the chosen sample number of each class
        :param transform: the transformation to be applied (data augmentation)
        :return:
        """

        total_cls = self.labels.unique()
        choice = []
        for cls in total_cls:
            idx = torch.where(self.labels == cls)
            if size_per_cls > len(idx[0]):
                choice.append(np.array(idx[0].cpu()))
            else:
                choice.append(np.random.choice(np.array(idx[0].cpu()), size_per_cls, replace=False))
