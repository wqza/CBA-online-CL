...
def get_parser() -> ArgumentParser:
    ...
    parser.add_argument('--minlr', type=float, default=5e-4,
                        help='Penalty weight.')
    parser.add_argument('--fitting_epochs', type=int, default=128,
                        help='Penalty weight.')
...
def fit_buffer(self, epochs):
    for epoch in range(epochs):
        ...
        all_inputs, all_labels = self.buffer.get_data(len(self.buffer.examples), transform=self.transform)
    ...
    #     loss.backward()
    #     optimizer.step()
    # progress_bar(0, epochs, 1, 'G', loss.item())
