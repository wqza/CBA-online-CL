...
class MetaModule(nn.Module):
    ...
    def update_params(self, lr_inner, first_order=False, source_params=None, detach=False):
        if isinstance(lr_inner, list):
            # WQZA: Used for updating parameters with different learning rate
            if source_params is not None:
                for tgt, src, lr in zip(self.named_params(self), source_params, lr_inner):
                    name_t, param_t = tgt
                    grad = src
                    if first_order:
                ...
                for name_param, lr in zip(self.named_params(self), lr_inner):
            ...
            # Origin code of the function 'update_params'
            if source_params is not None:
...
class MetaLinear(MetaModule):
    def __init__(self, *args, **kwargs):
        ...
        if 'bias' in kwargs and not kwargs['bias']:
